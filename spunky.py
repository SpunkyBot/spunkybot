    ...
    def handle_userinfo(self, line):
        ...
        with self.players_lock:
            ...
            if player_num not in self.game.players:
                ...
                if player_ban_id:
                    self.kick_player_reason("^7%s ^1banned ^7(ID @%s)" % (player.get_name(), player_ban_id), player_num)
                else:
...
## mod level 20
            ...
            # country / locate
            elif (sar['command'] == '!country' or sar['command'] == '@country' or sar['command'] == '!locate' or sar['command'] == '@locate' or sar['command'] == '!lc') and self.game.players[sar['player_num']].get_admin_role() >= 20:
                if line.split(sar['command'])[1]:
